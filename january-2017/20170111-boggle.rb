    ...
    for i in 0...rows do
      for j in 0...columns do
        if boggle[i][j] == dictionary[k][0]
          if dfs_for_letters(boggle, dictionary[k], i, j)
...
def dfs_for_letters(boggle, word, row, column, visited = [])
  visited = Array.new(boggle.length, Array.new(boggle.first.length), false)
...
def is_safe?(boggle, visited, i, j, letter)
